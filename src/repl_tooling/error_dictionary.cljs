...
(ns repl-tooling.error-dictionary
(:require [repl-tooling.dictionaries :as dictionaries]))
...
(def error-dictionary
  [
   ...
   {:key :class-cast-exception
    ...
    :match (dictionaries/beginandend "Cannot cast (\\S*) to (\\S*)")
    ...
    {:key :class-cast-exception-lower-case
     :class "ClassCastException"
     :match (dictionaries/beginandend "(\\S*) cannot be cast to (\\S*)")
     :fn (fn [matches] (str "Expected "
                                           (get-type (nth matches 2)) ", but "
                                           (get-type (nth matches 1)) " was given instead.\n"))}
    ...
    {:key :class-cast-exception-with-class
     :class "ClassCastException"
     :match (dictionaries/beginandend "class (\\S*) cannot be cast to class (\\S*)")
     :fn (fn [matches] (str "Expected "
                                          (get-type (nth matches 2)) ", but "
                                          (get-type (nth matches 1)) " was given instead.\n"))}
    ...
    :match (dictionaries/beginandend "assoc expects even number of arguments after map/vector, found odd number")
    ...
    :class "IllegalArgumentException"
    :match (dictionaries/beginandend "Wrong number of args passed to keyword: (\\S*)")
    ...
    :class "IllegalArgumentException"
    :match (dictionaries/beginandend "Key must be integer(\\S*)")
    :fn (fn [matches] "An argument for a vector must be an integer number.\n")}

    {:key :illegal-argument-no-val-supplied-for-key
    :class "IllegalArgumentException"
    :match (dictionaries/beginandend "No value supplied for key: (\\S*)")
    ...
    :class "IllegalArgumentException"
    :match (dictionaries/beginandend "Vector arg to map conj must be a pair")
    ...
    {:key :cant-call-nil
    :class "IllegalArgumentException"
    :match (dictionaries/beginandend "Can't call nil, form: \\((.*)\\)")
    ...
    :class "IllegalArgumentException"
    :match (dictionaries/beginandend "Duplicate key: (\\S*)")
    ...
    {:key :loop-req-vector
    :class "IllegalArgumentException"
    :match (dictionaries/beginandend "loop requires a vector for its binding")
    :fn (fn [matches] (str "Loop requires a vector for its binding.\n"))}

    {:key :other-req-vector
    :class "IllegalArgumentException"
    :match (dictionaries/beginandend "(\\S+) requires a vector for its binding")
    :fn (fn [matches] (str (get-function-name (nth matches 1))
                                          " requires a vector for its binding.\n"))}

    {:key :recur-arg-mismatch
    :class "IllegalArgumentException"
    :match (dictionaries/beginandend #"Mismatched argument count to recur, expected: (.*) args, got: (.*)")
    ...
    :class "IllegalArgumentException"
    :match (dictionaries/beginandend #"No matching clause: (.*)")
    ...
    :class "IllegalArgumentException"
    :match (dictionaries/beginandend "Cannot open \\<(.*)\\> as an InputStream")
    :fn (fn [matches] (str (nth matches 1)
                                          " cannot be opened as an InputStream.\n"))}

   {:key :illegal-input-must-be-x
   :class "IllegalArgumentException"
   :match (dictionaries/beginandend "Argument must be an (.*): (.*)  (.*)\\.(.*)\\.")
   ...
   :class "IllegalArgumentException"
   :match (dictionaries/beginandend #"(\S*) not supported on type: (\S+)")
   :fn (fn [matches] (str "The function "
    ...
    {:key :no-ctor-found
    :class "IllegalArgumentException"
    :match (dictionaries/beginandend #"No matching ctor found for class (.+)")
    ...
    {:key :no-method-found
    :class "IllegalArgumentException"
    :match (dictionaries/beginandend #"No matching method (.+) found taking (\d+) args for class (.+)")
    ...
    {:key :no-field-found
    :class "IllegalArgumentException"
    :match (dictionaries/beginandend #"No matching field found: (.+) for class (.+)")
    :fn (fn [matches] (str "There is no method "
                                          (nth matches 1)
    ...
    :class "IllegalArgumentException"
    :match (dictionaries/beginandend #"Argument must be an integer: clojure\.lang\.LazySeq(.*)")
    :fn (fn [matches] (str "Expected an integer number, but a sequence was given instead.\n"))}

    {:key :illegal-arg-must-be-int-seq
    :class "IllegalArgumentException"
    :match (dictionaries/beginandend #"Argument must be an integer: (.*)")
...
;; TODO: fix those! Issue #103
   #_{:key :assertion-error-with-argument
    ...
    :match (dictionaries/beginandend "Assert failed: \\((\\S*) argument(\\S*)\\)")
   ...
   {:key :assertion-error-without-argument
    :class "AssertionError"
    :match (dictionaries/beginandend "Assert failed: (\\S*)")
    ...
    :match (dictionaries/beginandend "Wrong number of args \\((\\S*)\\) passed to: core/keyword")
    ...
    :class "ArityException"
    :match (dictionaries/beginandend "Wrong number of args \\((\\S*)\\) passed to: core/(\\S*)")
    ...
    ;we may want to find a way to make this less general
    :class "ArityException"
    :match (dictionaries/beginandend "Wrong number of args \\(1\\) passed to: (\\S+)")
    ...
    ;we may want to find a way to make this less general
    :class "ArityException"
    :match (dictionaries/beginandend "Wrong number of args \\((\\S*)\\) passed to: (\\S*)")
    :fn (fn [matches] (str (check-function-name (get-function-name (nth matches 2)))
   ...
   {:key :compiler-exception-cannot-resolve-symbol-if
    ...
    :match (dictionaries/beginandend "Unable to resolve symbol: if in this context")
    :fn (fn [matches] (str "You are not using if correctly.\n"))}

  {:key :compiler-exception-cannot-resolve-symbol
   :class "RuntimeException"
   :match (dictionaries/beginandend "Unable to resolve symbol: (.+) in this context")
...
;;
   ...
   {:key :arithmetic-exception-divide-by-zero
    ...
    :match (dictionaries/beginandend "Divide by zero")
   ...
   ;######################################
    ...
    :match (dictionaries/beginandend "String index out of range: (\\S+)")
    ...
    :match (dictionaries/beginandend "") ; an empty message
   ...
   {:key :null-pointer-non-existing-object-provided
    ...
    :match (dictionaries/beginandend "(.+)") ; for some reason (.*) matches twice. Since we know there is at least one symbol, + is fine
    ...
    :class "NullPointerException"
    :match  (dictionaries/beginandend "")
    ...
    :match (dictionaries/beginandend "(\\S*) not supported on this type: (\\S*)")
    ...
    :class "UnsupportedOperationException"
    :match (dictionaries/beginandend "Can only recur from tail position")
   ...
   ;##############################
    ...
    :match (dictionaries/beginandend "(\\S*)")
    :fn (fn [matches] (str "Name "
                           (nth matches 1)
                           " is undefined.\n"))}
   ...
   ;###############################
    ...
    :match (dictionaries/beginandend "Invalid number: (\\S*)")
    ...
    :match (dictionaries/beginandend "Illegal field name (\\S*) in class (\\S*)")
    ...
    {:key :compiler-exception-map-literal-even
     :class "RuntimeException"
     :match (dictionaries/beginandend "Map literal must contain an even number of forms")
    ...
    {:key :compiler-exception-unmatched-delimiter
     :class "RuntimeException"
     :match (dictionaries/beginandend "Unmatched delimiter: (\\S*)")
    ...
    {:key :compiler-exception-too-many-arguments
     :class "RuntimeException"
     :match (dictionaries/beginandend "Too many arguments to (\\S*)")
   ...
   {:key :compiler-exception-too-few-arguments
    :class "RuntimeException"
    :match (dictionaries/beginandend "Too few arguments to (\\S*)")
    ...
    {:key :compiler-exception-end-of-file
     :class "RuntimeException"
     :match (dictionaries/beginandend "EOF while reading, starting at line (\\d+) ")
    ...
    ;this cannot be done in repl needs to be tested still
     :class "RuntimeException"
     :match (dictionaries/beginandend "EOF while reading string")
    ...
    ;This error message needs to be improved
     :class "RuntimeException"
     :match (dictionaries/beginandend "EOF while reading")
    ...
    {:key :compiler-exception-no-such-var
     :class "RuntimeException"
     :match (dictionaries/beginandend "No such var: (\\S*)/(\\S*),")
   ...
   {:key :compiler-exception-same-arity
    :class "RuntimeException"
    :match (dictionaries/beginandend "Can't have 2 overloads with same arity")
    ...
    ;This error message needs to be improved
     :class "UnsupportedOperationException"
     :match (dictionaries/beginandend "Can only recur from tail")
    ...
    {:key :out-of-memory
     ...
     :match (dictionaries/beginandend "Java heap space")
    ...
    {:key :file-does-not-exist
     ...
     :match (dictionaries/beginandend "(.*) \\(No such file or directory\\)")
    ...
    {:key :file-does-not-exist-windows
     :class "FileNotFoundException"
     :match (dictionaries/beginandend "(.*) \\(The system cannot find the file specified\\)")
     :fn (fn [matches] (str "The file "
                                           (nth matches 1)
                                           " does not exist.\n"))}
    ...
    {:key :file-not-found-on-load
     :class "FileNotFoundException"
     :match (dictionaries/beginandend "Could not locate (\\S+).class or (\\S+).clj on classpath(.*)load")
    ...
    {:key :other
     :class "WARNING:"
     :match (dictionaries/beginandend "(\\S*) already refers to: (\\S*) in namespace: (\\S*), being replaced by: (\\S*)")
